...
describe Absa::H2h::Transmission::Eft::Header do
  ...
  it "should validate the total value of debit transactions and any credit contra records in the trailer" do
    ...
    @user_set[:trailer][:no_credit_records] = "2"
    @user_set[:trailer][:no_contra_records] = "2"
  ...
  it "should validate the total value of credit transactions and any debit contra records in the trailer" do
    @additional_transactions.each do |t|
      t[:content][:bankserv_record_identifier] = "10" if t[:type] == "standard_record"
      @user_set[:transactions] << t
    end
    @user_set[:transactions].last[:content][:amount] = "20500"
    @user_set[:trailer][:last_sequence_number] = "7"
    @user_set[:trailer][:no_debit_records] = "1"
    @user_set[:trailer][:no_credit_records] = "6"
    @user_set[:trailer][:no_contra_records] = "2"
