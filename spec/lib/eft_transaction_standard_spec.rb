...
describe Absa::H2h::Transmission::Eft::StandardRecord do
  before(:each) do
    ...
    @eft_transaction = @hash[:transmission][:sections].first[:content][:transactions].first[:content]
  end
  
  it "should be able to build an eft transaction record" do
    trailer = Absa::H2h::Transmission::Eft::StandardRecord.new(@eft_transaction)
